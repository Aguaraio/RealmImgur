...
-   No longer assert on a "!cfg.session_initiator"
    in SlabAlloc::attach_file(). This makes issue #1784 go away, but
    also removes an option to detect and flag if the ".lock" file is
    deleted while a SharedGroup is attached to the file. Please note:
    Removal of the ".lock" file while the database is attached may lead
...
-   S: A new Apple CoreFoundation implementation of event loop API
    was added.
...
b)  In a multiprocess setting, another process would be allowed to run
    ...
    the !cfg.session_initiator would not be triggered. May fix
    issue #1782.
...
-   Fix TableView::clear() to work in imperative mode (issue
    #1803, #827)
...
-   Fix handing over an out of sync TableView that depends on a deleted
    link list or row so that it doesn't remain perpetually out of
    sync (#1770).
...
-   Fix for: The commit logs were not properly unmapped and closed when
    ...
    through a different SharedGroup would operate on a different set
    of files. This could cause inconsistency between the commit log and
    the database. In turn, this could lead to crashes during
    advance_read(), promote_to_write() and possibly
    commit_and_continue_as_read(). Worse, It could also silently lead to
    accessors pointing to wrong objects which might later open for
    changes to the database that would be percieved as corrupting.
    (#1762)
-   Fix for: When commitlogs change in size, all readers (and writers)
    ...
    this bug could be crashes in advance_read(), promote_to_write()
    or commit_and_continue_as_read(). Conceivably it could also cause
    wrong accessor updates leading to accessors pointing to wrong
    database objects. This, in turn, could lead to what would be
    percieved as database corruption. (#1764)
...
-   Fix of #1605 (LinkView destruction/creation should be thread-safe)
    and most likely also #1566 (crash
    below LinkListColumn::discard_child_accessors...) and possibly also
    #1164 (crash in SharedGroup destructor on OS X).
-   Copying a Query restricted by a TableView will now avoid creating a
    dangling reference to the restricting view if the query owns
    the view. Dangling references may still occur if the Query does not
    own the restricting TableView.
...
-   New features in the unit test framework: Ability to log to a file
    ...
    each unit test.
-   New failure simulation features: Ability to prime for
    random triggering.
...
-   Copying a Query restricted by a TableView will now avoid creating a
    dangling reference to the restricting view if the query owns
    the view. Dangling references may still occur if the Query does not
    own the restricting TableView. (#1741)
...
-   Bumps table version when Table::set_null() called.
    ...
    changes after Table::set_null() was
    called. (https://github.com/realm/realm-java/issues/2366)
...
-   Language bindings can now test if a TableView depends on a deleted
    ...
    https://github.com/realm/realm-core/issues/1509 and
    also TEST(Query_ReferDeletedLinkView) in test_query.cpp for details.
-   LangBindHelper::advance_read() and friends no longer take a
    history argument. Access to the history is now gained automatically
    via Replication::get_history(). Applications and bindings should
    simply delete the history argument at each call site.
...
-   While generating transaction logs, we didn't always deselect
    nested accessors. For example, when performing a table-level
    operation, we didn't deselect a selected link list. In some cases,
    it didn't matter, but in others it did. The general rule is that an
    operation on a particular level must deselect every accessor at
    deeper (more nested) levels. This is important for the merge logic
    of the sync mechanism, and for transaction log reversal. This has
    been fixed.
-   While reversing transaction logs, group level operations did not
    terminate the preceding section of table level operations.
    Was fixed.
...
-   Important for language bindings: Any method on Query and TableView
    ...
    longer throw DeletedLinkView or crash.
    See TEST(Query_ReferDeletedLinkView) in test_query.cpp for
    more info.
...
-   Memory errors caused by calls to mmap/mremap will now throw a
    ...
    handle it differently than the rest of the
    general std::runtime_errors.
...
-   Regular assertions (REALM_ASSERT()) are no longer enabled by default
    ...
    no longer needs to explicitely disable regular assertions in
    release mode.
-   Upgraded Android toolchain to R10E and gcc to 4.9 for
    all architectures.
...
-   Primary key support has been removed. Instead, new instructions have
    ...
    these instructions in place of the regular SetInt and
...
-   Added TableView::distinct() method. It obeys
    ...
    populate the TableView and then perform a distinct algorithm on
    that (i.e. it will _not_ add a secondary distinct filter to any
    earlier filter applied). See more in TEST(TableView_Distinct)
    in test_table_view.cpp.
...
-   New substring operations (ranged insert, erase on values in
    string columns).
...
-   New operations: Swap rows, move rows, move column, move group
    level table.
...
-   New assertion macro: REALM_ASSERT_EX, replacing
    REALM_ASSERT_n macros.
...
-   Fixed all aggregate methods on Table (min, max, etc) that hadn't
    been updated/kept in sync for a long while (null support,
    return_ndx argument,..).
...
-   A stackoverflow issue in encrypted_file_mapping. Allocing 4k bytes
    on the stack would cause some random crashes on small stack
    size configurations.
...
-   A bug in Query copying has been fixed. The bug could cause
    ...
    SharedGroup to leak to another during handover_export() leading
    to corruption.
...
-   TableView::remove(), TableView::remove_last(), and
    ...
    does not maintain the order of the remaining rows in the
    underlying table. In any case, the order of remaining rows in the
    table view is maintained. This is listed as an API breaking change
    because the situation before this change was confusing, to say
    the least. In particular, TableView::clear() would choose between
    the ordered and the unordered mode based on whether the underlying
    table had at least one link (or link list) column. You are strongly
    advised to revisit all call sites and check that they do the
    right thing. Note that both bindings (Cocoa and Android) are likely
    to want to use unordered mode everywhere.
...
-   Added argument to SharedGroup to prevent automatic file
    format upgrade. If an upgrade is required, the constructor will
...
-   SharedGroup no longer needs to remap the database file when
    it grows. This is a key requirement for reusing the memory mapping
    across threads.
-   NOEXCEPT* macros have been replaced by the C++11 noexcept specifier.
-   The REALM_CONSTEXPR macro has been replaced by the C++11
    constexpr keyword.
...
-   Reverted prelinking of static libraries on Apple platforms as it
    ...
    scenarios, including when sorting by integer or floating
    point columns.
...
-   Virtual ColumnBase::insert(row_ndx, num_rows, is_append) was changed
    ...
    information than is_append/is_last. num_rows_to_erase was added
    for consistency.
...
-   Support for transfer/handover of TableViews, Queries, ListViews and
    ...
    is treated.
-   Support for non-end row insertion in tables with link and link
    list columns.
...
-   Improved exception safety in connection with the initiation and
    termination of transactions.
-   Add support for systems where mremap() exists but fails
    with ENOTSUP.
...
-   The check for functioning SEGV signals threw the exception
    only once. Now it always throws when trying to use encryption.
...
-   Fixed durability issue in case of power / system failures on
    Apple platforms. We now use a stronger synchronization
    (fcntl(fd, F_FULLFSYNC)) to stable storage when the file
    is extended.
...
-   Changes the mmap doubling treshold on mobile devices from 128MB
    to 16MB.
...
-   Fixed crash when std::exit() is called with active
    encrypted mappings.
...
-   Fixed out-of-bounds reads when using aggregate functions on
    sorted TableViews.
...
-   Add support for comparing string columns to other string columns
    in queries.
-   WriteTransaction::has_table() and WriteTransaction::rollback()
    were added. Previously, only implicit rollback was possible with
    WriteTransaction.
...
-   Fixed a crashbug which could cause a reading thread to miss accessor
    ...
    datastructures which could presumably later cause
    database corruption.
...
-   SharedGroup now allows you to coordinate which version of the data a
    ...
    obtain a a transaction accessing that same version from
    another ShareGroup. This is done by new forms of
    SharedGroup::begin_read(), SharedGroup::advance_read(). Operators
    are provided so that VersionID's may be compared.
...
-   Fall back to the old query nodes for String as well
    as int/double/float.
-   Log assertions failures to the native logging system on android
    and Apple.
...
-   There is now three kinds of runtime assertions,
    ...
    release-mode builds too, depending on the specific
    build configuration.
-   REALM_ASSERT() assertions are now enabled in release-mode builds
    by default.
...
-   Fixed a bug where rollback of an empty transaction could cause
    a crash.
...
-   Table::erase() can no longer be used with unordered tables.
    ...
    the last row in the table. One must now always
    use Table::move_last_over() with unordered tables. Whether a table
    is ordered or unordered is entirely decided by the way it is used by
...
-   Added not_equal(), equal(), contains(), begins_with(), ends_with()
    ...
    table1->column(0).contains("A", true).find();'. Works with
    Links too.
...
-   Sorting on LinkView and TableView by multiple columns: Both classes
    ...
    multiple columns.
-   You can now query a LinkView by
    calling Query::where(link_view.get())....
    See TEST(LinkList_QueryOnLinkList) in test_link_query_view.cpp for
    an example. *** IMPORTANT NOTE: Do not call sort() on a LinkView
    because it does not yet support replication ***. get_sorted_view()
    works fine though.
...
-   Fixed bug in deep copy of Query, causing the experienced crash at
    end of scope of a Query after add_constraint_to_query() had
    been executed. The fix may not be optimal as it limits nesting of
...
-   It's now possible to sort a LinkRef according to a column in the
    target table. Also lets you build a TableView with the sorted
    result instead. The new methods on LinkViewRef are sort() and
    get_sorted_view()
...
-   Fixed bug in TableView::remove() causing crash or
    undefined behavior.
...
-   Fixed in Array::erase() causing crash in certain row
    insertion scenarios.
...
-   Now supports DateTime Query::maximum_datetime() and
    DateTime Query::minimum_datetime()
...
-   New test suite with support for parallelized testing
    replaces UnitTest++. See section 'Testing' in README.md.
...
in <realm/string_data.hpp>. 'StringData' can be implicitly constructed
from 'const char_', so no change is required when passing arguments.
Source code change is required when dealing with returned strings of
type 'const char_'. The following is a complete list: Affected form
...
-   my_table[i].foo.set(...) added for all column types
    except subtables. This is for completeness.
...
-   16 New Table:get_string_length(col_ndx, row_ndx) added in
    Dynamic Table. Missing in Typed Table.
...
-   15 Updated Group() constructor to take an optional 3'rd parameter
    ...
    provided data: Group::Group(const char* buffer, size_t len,
...
-   10 Renamed FindAllMulti() to find_all_multe(). And SetThreads()
    to set_threads()
...
-   New macro REALM_DEBUG to control compilation mode. The library
    ...
    1)  Define REALM_DEBUG in any translation unit that includes a
        Realm header.
    2)  Use the version of the library that is compiled for debug
        mode (librealm_d.a).
-   5 Removed obsolete constructor Mixed(ColumnType).
    Use Mixed(subtable_tag) instead, since this has no runtime overhead.
...
-   Destroying a Group will invalidate all table wrappers (instances
    of Table) as well as all direct and indirect subtable wrappers.
...
-   Added a the following methods to a TableView: template void
    ...
    column_ndx, time_t value) const; (for class TableView
    and ConstTableView)
